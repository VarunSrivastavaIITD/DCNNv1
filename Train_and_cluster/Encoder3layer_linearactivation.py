...
class MyNet(nn.Module):
    def __init__(self, window = 200, subwindow = 40, dropout = 0):
        ...
        c1 = nn.Sequential(nn.Conv1d(1, 10, 7, padding = 3), nn.LeakyReLU())
        c2 = nn.Sequential(nn.Conv1d(10, 10, 5, padding = 2), nn.LeakyReLU())
        c3 = nn.Sequential(nn.Conv1d(10, 1, 3, padding = 1))

        self.feature_extractor = nn.Sequential(c1, c2, c3)

        # n, 50
        self.classifier = nn.Sequential(
            nn.Linear(200, 150), nn.SELU(), nn.AlphaDropout(dropout),
    ...
    def forward(self, x):
        # Feature extractor
        x3 = self.feature_extractor(x)

        # Flatten
        x3 = x3.view(x3.size(0), -1)

        x1 = self.classifier(x3)
        x2 = self.regressor(x3)
        y = th.squeeze(th.cat((x1, x2, x3), 1))
...
def custom_loader(speechfolder,
    ...
    #     egg_data[i] = egg_data[i] / (np.linalg.norm(egg_data[i]) + 1e-10)
    ...
    return speech_windowed_data, peak_distance, peak_indicator, egg_windowed_data
...
def create_dataloader(batch_size,
    ...
    speech_windowed_data, peak_distance, peak_indicator, egg_windowed_data = custom_loader(
        speechfolder,
        peakfolder,
        eggfolder,
        window,
...
def train(model: nn.Module,
    ...
    reconstruction_weight = Variable(th.Tensor([reconstruction_weight]))
    ...
    locn_start = int(float(window / 2) - float(subwindow / 2))
    locn_end = int(float(window / 2) + float(subwindow / 2))
    ...
    for data, target, egg_data in train_data:
        if scheduler is not None:
            scheduler.step()

        if use_cuda:
            data, target, egg_data = data.cuda(), target.cuda(), egg_data.cuda()
            bce_weight = bce_weight.cuda()
            mse_weight = mse_weight.cuda()
            reconstruction_weight = reconstruction_weight.cuda()
            misclass_weight = misclass_weight.cuda()
            corclass_weight = corclass_weight.cuda()
            # thresh_val = thresh.cuda()
            gci_thresh = gci_thresh.cuda()

        data, target, egg_data = Variable(data), Variable(target), Variable(egg_data)
        optimizer.zero_grad()
        ...
        gci_misclass_temp = (1 - peak_indicator_target) * out
        gci_misclass += gci_misclass_temp.mean().item()

        out = (F.sigmoid(probabilities) > gci_thresh).float()
        gci_misses_temp = peak_indicator_target * (1 - out)
        gci_misses += gci_misses_temp.mean().item()

        loss_corrclass = peak_indicator_target * (
            (1 - F.sigmoid(probabilities))**2)
        loss_corrclass = loss_corrclass.mean()

        loss_reconstruction = th.abs(egg_data - reconstructions).sum(dim = 1)
        loss_reconstruction = loss_reconstruction.mean()

        net_loss = bce_weight * loss_bce + mse_weight * loss_mse + reconstruction_weight * loss_reconstruction
        ...
        optimizer.step()
    del loss_mse, loss_bce, loss_corrclass, loss_misclass, net_loss, loss_reconstruction
    th.cuda.empty_cache()
    ...
    return loss_sum / batches, bce_loss / batches, mse_loss / batches, reconstruction_loss / batches, gci_misclass / batches, gci_misses / batches
...
def test(model: nn.Module,
    ...
    for data, target, egg_data in test_loader:
        if use_cuda:
            data, target, egg_data = data.cuda(), target.cuda(), egg_data.cuda()
        data, target, egg_data = Variable(data, volatile=True), Variable(target), Variable(egg_data)
        output = model(data)
    ...
    print(
        '\nTest set: BCE loss: {:.4f} MSE loss: {:.4f} Reconstruction loss {:.4f} Mean Batch Misclassification {:.4f} GCI Misses {:.4f}\n'.
        format(bce_loss, mse_loss, reconstruction_loss, mean_misclass, gci_misclass))

def main():
    train_data = create_dataloader(
        32768,
        "CMU_new/bdl_npy_voiced/speech",
        "CMU_new/bdl_npy_voiced/degg",
        "CMU_new/bdl_npy_voiced/egg",
        200,
        3,
        select=100)
    ...
    with warnings.catch_warnings():
        ...
        for i in range(epochs):
            ...
            nloss, bloss, mloss, r_loss, gci, miss = train(model, optimizer,
                                                   train_data, use_cuda)
            print(
                'Train Net Loss: {:.4f} BCE Loss {:.4f} MSE Loss {:.4f} Reconstruction Loss {:.4f} Mean Batch Misclassification {:.4f} GCI Misses {:.4f} @epoch {}'.
                format(nloss, bloss, mloss, r_loss, gci, miss, i))
